...
export default defineEventHandler(async (event: H3Event) => {
  ...
  const pageMatch = pageParam.match(/product-sitemap-(\d+)/)
  ...
  const response = await fetch(`${sanctumBaseUrl}/api/products-sitemap-urls?page=${page}`)
  const data = await response.json()

  const productUrls = data.data.map((item: { loc: string, lastmod: string, 'image:loc'?: string, title?: string }) => ({
    loc: item.loc,
