...
export default defineEventHandler(async (event: H3Event) => {
  ...
  const pageMatch = pageParam.match(/brand-sitemap-(\d+)/)
  ...
  const response = await fetch(`${sanctumBaseUrl}/api/brands-sitemap-urls?page=${page}`)
  const data = await response.json()

  const brandUrls = data.data.map((item: { loc: string, lastmod: string, 'image:loc'?: string, title?: string }) => ({
    loc: item.loc,
